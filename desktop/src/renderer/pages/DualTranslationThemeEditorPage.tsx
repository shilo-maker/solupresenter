...
const DEFAULT_LINE_POSITIONS: Record<string, LinePosition> = {
  original: {
    x: 0, y: 20, width: 100, height: 11,
    paddingTop: 0, paddingBottom: 0,
    alignH: 'center', alignV: 'center'
  },
  transliteration: {
    x: 0, y: 31, width: 100, height: 10,
    paddingTop: 0, paddingBottom: 0,
    alignH: 'center', alignV: 'center'
  },
  translation: {
    x: 0, y: 41, width: 100, height: 10,
    paddingTop: 0, paddingBottom: 0,
    alignH: 'center', alignV: 'center'
  },
  translationB: {
    x: 0, y: 51, width: 100, height: 10,
    paddingTop: 0, paddingBottom: 0,
    alignH: 'center', alignV: 'center'
  }
};
...
const DEFAULT_LINE_STYLES: Record<string, LineStyle> = {
  original: {
    fontSize: 195, fontWeight: '700', color: '#ffffff', opacity: 1, visible: true
  },
  transliteration: {
    fontSize: 150, fontWeight: '400', color: '#ffffff', opacity: 0.9, visible: true
  },
  translation: {
    fontSize: 160, fontWeight: '400', color: '#cfcfcf', opacity: 0.85, visible: true
  },
  translationB: {
    fontSize: 160, fontWeight: '400', color: '#a0c8e0', opacity: 0.85, visible: true
  }
};

const DualTranslationThemeEditorPage: React.FC = () => {
  ...
  const [theme, setTheme] = useState<Theme>({
    id: '',
    name: 'New Dual Translation Theme',
    isBuiltIn: false,
    viewerBackground: { type: 'color', color: '#000000' },
    canvasDimensions: { width: 1920, height: 1080 },
    lineOrder: ['original', 'transliteration', 'translation', 'translationB'],
  ...
  useEffect(() => {
    if (themeId) {
      window.electronAPI.getDualTranslationTheme(themeId).then((loadedTheme: any) => {
        if (loadedTheme) {
          setTheme({
            id: loadedTheme.id || '',
            ...
            lineOrder: loadedTheme.lineOrder || ['original', 'transliteration', 'translation', 'translationB'],
        ...
        console.error('Failed to load dual translation theme:', error);
      });
    }
  }, [themeId]);
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      if (theme.id) {
        await window.electronAPI.updateDualTranslationTheme(theme.id, themeData);
      } else {
        const created = await window.electronAPI.createDualTranslationTheme(themeData);
      ...
      console.error('Failed to save dual translation theme:', error);
  ...
  const selectedLineType = selectedElement?.type === 'line' ? selectedElement.id : null;
    ...
    <div style={{
      ...
      <div style={{
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: '16px' }}>
          ...
          />
          <span style={{ fontSize: '12px', color: 'rgba(255,255,255,0.5)' }}>Dual Translation Theme (4 lines)</span>
          {hasChanges && (
                      ...
                      return (
                        ...
                        >
                          ...
                          <span style={{
                            ...
                            {lineType === 'translationB' ? 'Translation B' : lineType}
                          </span>
                        </div>
                      );
                    })}
...
export default DualTranslationThemeEditorPage;
